...
class wavePlayer:
    def __init__(self,leftPin=Pin(12),rightPin=Pin(12), virtualGndPin=None,
    ...
    def stop(self):
        ...
        self.dma1.abort()
        self.leftPWM.duty(0)
        self.rightPWM.duty(0)

    def play(self,filename, keyboard):
        result = "finished"
...
#         print("channels:", channels, ", rate:", rate, ", bytesDepth:", bytesDepth, ", frameCount:", frameCount)
...
#         self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= rate // 2000, clock_DIV=62500)
        if rate == 36000:
            self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=7500) # 32k
        elif rate == 24000:
            self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=10000) # 24k
        elif rate == 16000:
            self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=15000) # 16k
        elif rate == 8000:
            self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=30000) # 8k
#         self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=15000) # 16k
#         self.dma0 = myDMA(self.dma0Channel,timer=self.dmaTimer,clock_MUL= 1, clock_DIV=30000) # 8k
        ...
        while frameLeft>0 and keyboard.scan() != "ES":
            try:
                # first DMA
    ...
    #                 self.leftPWM.duty(self.PWM_TOP)
    #                 self.rightPWM.duty(self.PWM_TOP)
