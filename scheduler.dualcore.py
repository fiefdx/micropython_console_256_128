...
class Message(object):
    pool = []
    lock = thread.allocate_lock() if thread else None
...
class Scheluder(object):
    def __init__(self, log_to = None, name = "scheduler", cpu = 0, s = None):
        ...
        self.task_sleep_interval = const(0.01)
                        ...
                        if peek.ready():
                            ...
                            try:
                                ...
                                for msg in self.current.condition.send_msgs:
                                    ...
                                    if self.s:
                                        self.s.lock.acquire()
                                        if msg.receiver in self.s.tasks_ids:
                                            self.s.tasks_ids[msg.receiver].put_message(msg)
                                        self.s.lock.release()
                                self.current = None
                                self.need_to_sort = True
