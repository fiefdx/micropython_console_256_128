...
def render_tiles(name, msg, tile_wri):
    ...
    size_w = msg.content[name]["size_w"]
    size_h = msg.content[name]["size_h"]
...
def render_objects(name, msg, tile_wri):
    for o in msg.content[name]:
        tile_wri.print_tile_id(o["id"], o["x"], o["y"])


def render_bricks(name, msg, tile_wri):
    offset_x = msg.content[name]["offset_x"]
    offset_y = msg.content[name]["offset_y"]
    width = msg.content[name]["width"]
    height = msg.content[name]["height"]
...
def render_texts(name, msg, wri, lcd):
...
def render(category, msg, wri, tile_wri, lcd, refresh):
    ...
    renders = {
        "tiles": (render_tiles, [name, msg, tile_wri]),
        "objects": (render_objects, [name, msg, tile_wri]),
        "bricks": (render_bricks, [name, msg, tile_wri]),
        "texts": (render_texts, [name, msg, wri, lcd]),
            ...
            try:
                ...
                if "render" in msg.content:
                    for category in msg.content["render"]:
                        refresh = render(category, msg, wri, tile_wri, lcd, refresh)
