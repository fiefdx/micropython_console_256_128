...
def run_core1(task, name, scheduler = None, start_after = 5000, display_id = None):
    ...
    yield Condition.get().load(send_msgs = [Message.get().load({"output": "start core1"}, receiver = display_id)])
...
if __name__ == "__main__":
    try:
        ...
        s = Scheluder(cpu = 0)
        s1 = None
        enable_core1 = const(True)
        if thread and enable_core1:
            s1 = Scheluder(cpu = 1, s = s)
        try:
            ...
            led.off()
            if s1:
                #sound_id = s1.add_task(Task.get().load(sound_output, "sound_output", condition = Condition.get(), kwargs = {"scheduler": s, "sound_pwm": sound_pwm}))
                #s1.sound_id = sound_id
                keyboard_id = s1.add_task(Task.get().load(keyboard_input, "keyboard_input", condition = Condition.get(), kwargs = {"scheduler": s, "interval": 30, "shell_id": shell_id, "display_id": display_id}))
